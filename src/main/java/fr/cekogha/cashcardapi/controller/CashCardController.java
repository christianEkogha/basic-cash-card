...
public class CashCardController {
	...
	public ResponseEntity<Void> createCashCard(@RequestBody CashCard newCashCard,
			UriComponentsBuilder uriComponentBuilder, Principal principal) {
		CashCard savedCashCard = repository.save(new CashCard(null, newCashCard.amount(), principal.getName()));
		URI locationSavedCashCard = uriComponentBuilder.path("cashcards/{id}").buildAndExpand(savedCashCard.id())
				.toUri();
	...
	public ResponseEntity<List<CashCard>> findAll(Pageable pageable, Principal principal) {
		Page<CashCard> page = repository.findByOwner(principal.getName(), PageRequest.of(pageable.getPageNumber(),
				pageable.getPageSize(), pageable.getSortOr(Sort.by(Sort.DEFAULT_DIRECTION.ASC, "amount"))));
		return ResponseEntity.ok(page.getContent());
	}

	@PutMapping("/{id}")
	public ResponseEntity<String> updateCashCard(@PathVariable Long id, @RequestBody CashCard cashCardUpdate,
			Principal principal) {
		CashCard cashCard = findCashCard(id, principal);
		if (cashCard == null)
			return ResponseEntity.notFound().build();
