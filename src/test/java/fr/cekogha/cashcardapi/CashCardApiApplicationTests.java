...
class CashCardApiApplicationTests {
	...
	public void shouldCreateANewCashCard() {
		...
		ResponseEntity<Void> createResponse = restTemplate.withBasicAuth("christian", "abc123")
				.postForEntity("/cashcards", newCashCard, Void.class);
		assertThat(createResponse.getStatusCode()).isEqualTo(HttpStatus.CREATED);

		URI locationOfNewCashCard = createResponse.getHeaders().getLocation();
		ResponseEntity<String> getResponse = restTemplate.withBasicAuth("christian", "abc123")
	...
	public void shouldReturnAllCashCardsWhenListIsRequested() {
		ResponseEntity<String> response = restTemplate.withBasicAuth("christian", "abc123").getForEntity("/cashcards",
				String.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
	...
	public void shouldNotReturnACashCardWhenUsingBadCredentials() {
		...
		response = restTemplate.withBasicAuth("christian", "BAD-PASSWORD").getForEntity("/cashcards/99", String.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.UNAUTHORIZED);
	}
	...
	public void shouldUpdateAnExistingCashCard() {
		...
		ResponseEntity<String> response = restTemplate.withBasicAuth("christian", "abc123").exchange("/cashcards/99",
				HttpMethod.PUT, updatedCashCard, String.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);

		response = restTemplate.withBasicAuth("christian", "abc123").getForEntity("/cashcards/99", String.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
		DocumentContext documentContext = JsonPath.parse(response.getBody());
		Number id = documentContext.read("$.id");
		assertThat(id).isEqualTo(99);
	...
	public void shouldNotUpdateACashCardThatDoesNotExist() {
		HttpEntity<CashCard> updatedCashCard = new HttpEntity<>(new CashCard(null, 19.99, null));
		ResponseEntity<String> response = restTemplate.withBasicAuth("christian", "abc123").exchange("/cashcards/99999",
				HttpMethod.PUT, updatedCashCard, String.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
	}
	...
	public void shouldDeleteAnExistingCashCard() {
		ResponseEntity<Void> response = restTemplate.withBasicAuth("christian", "abc123").exchange("/cashcards/99",
	...
	public void shouldNotDeleteACashCardThatDoesNotExist() {
		ResponseEntity<Void> response = restTemplate.withBasicAuth("christian", "abc123").exchange("/cashcards/99999",
				HttpMethod.DELETE, null, Void.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
	}
	...
	public void shouldNotAllowDeletionOfCashCardsTheyDoNotOwn() {
		ResponseEntity<Void> response = restTemplate.withBasicAuth("christian", "abc123").exchange("/cashcards/102",
				HttpMethod.DELETE, null, Void.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);

		ResponseEntity<String> getResponse = restTemplate.withBasicAuth("karl", "karlpassword")
				.getForEntity("/cashcards/102", String.class);
		assertThat(getResponse.getStatusCode()).isEqualTo(HttpStatus.OK);
	}
